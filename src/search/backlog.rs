...
pub trait Backlog {
    ...
    fn search<P: Pick>(space: Space, searcher: &Searcher) -> Option<Solution>;
}
    ...
    fn search<P: Pick>(space: Space, searcher: &Searcher) -> Option<Solution> {
        ...
        while let Some((choice, space)) = backlog.tasks.pop() {
            ...
            if let Ok(propagated) = searcher.branch(&choice, obj_opt, space) {
                match propagated {
                    Propagated::Fixed(space) => backlog.push_tasks::<P>(space),
...
impl Stack {
    fn push_tasks<P: Pick>(&mut self, space: Space) {
        ...
        for mutation in branch(&space.vars[pivot]) {
            self.tasks
